...
import pe.edu.pucp.techshopper.reportes.ReporteUtil;
...
public class MovimientosStock{
    ...
    public ArrayList<MovimientoStockDTO> listarTodosLosMovimientos() {
        ArrayList<MovimientoStockDTO> lista = movimientoBO.listarTodosLosMovimientos();

        DateTimeFormatter formato = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");

        for (MovimientoStockDTO m : lista) {
            if (m.getFechaMovimiento() != null) {
                m.setFechaMovimientoStr(m.getFechaMovimiento().format(formato));
            }
        }
    ...
    @WebMethod(operationName = "reporteMovStocks")
    public byte[] reporteMovStocks (){
        return ReporteUtil.reporteMovStocks();
    }
