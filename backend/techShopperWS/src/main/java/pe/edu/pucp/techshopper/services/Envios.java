...
public class Envios {
    ...
    public ArrayList<EnvioDTO> listarTodosLosEnvios() throws RuntimeException {
        try {
            ArrayList<EnvioDTO> lista = envioBO.listarTodosLosEnvios();

            for (EnvioDTO e : lista) {
                if (e.getFechaEntrega() != null) {
                    e.setFechaEntregaStr(e.getFechaEntrega().toString()); // o usar un formato personalizado si deseas
                }
            }
